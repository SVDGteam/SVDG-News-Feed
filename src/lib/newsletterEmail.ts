...
function scoreBadge(score: number): { label: string; bg: string; fg: string } {
  ...
  switch (tier) {
    case 'High':
      return { label: 'HIGH', bg: COLORS.sky, fg: COLORS.bg }
    case 'Medium-High':
      return { label: 'MED-HIGH', bg: COLORS.blue, fg: COLORS.white }
    case 'Medium':
      return { label: 'MEDIUM', bg: COLORS.yellow, fg: COLORS.bg }
    default:
      return { label: 'LOW', bg: COLORS.low, fg: COLORS.french }
  }
}
            ...
            <tr>
              <td style="padding: 16px 28px${isLast ? ' 24px 28px' : ''};">
                <table role="presentation" width="100%" cellpadding="0" cellspacing="0">
                  <tr>
                    <td>
                      <span style="display:inline-block; background-color:${badge.bg}; color:${badge.fg}; font-family:'Courier New',monospace; font-size:10px; font-weight:700; letter-spacing:1px; border-radius:4px; padding:2px 6px; margin-bottom:6px;">SCORE ${score} · ${badge.label}</span>
          ...
          <table role="presentation" width="600" cellpadding="0" cellspacing="0" style="max-width:600px; width:100%; background-color:${COLORS.card}; border:1px solid ${COLORS.border}; border-radius:12px; overflow:hidden;" bgcolor="${COLORS.card}">
            ...
            <tr>
              <td style="padding: 28px 28px 0 28px; text-align:center;" bgcolor="${COLORS.card}">
                <span style="display:inline-block; background:#ffffff; border-radius:10px; padding:10px 14px; font-family: 'Courier New', monospace; font-weight:700; font-size:13px; letter-spacing:1px; color:${COLORS.bg};">SVDG</span>
                <div style="margin-top:14px; font-family: 'Courier New', monospace; font-size:11px; letter-spacing:2px; text-transform:uppercase; color:${COLORS.sky};">Newsletter</div>
